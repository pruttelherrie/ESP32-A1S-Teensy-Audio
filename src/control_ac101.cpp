...
//static const char *TAG = "AudioControlAC101";

bool AudioControlAC101::configured = false;

// --------- uit blackstomp main.cpp
#include <Wire.h>
#include <Arduino.h>
...
bool AudioControlAC101::LeftLineLeft(bool select)
{
...
bool AudioControlAC101::RightLineRight(bool select)
{
	uint16_t val = ReadReg(ADC_SRC);
	if(select)
...
bool AudioControlAC101::WriteReg(uint8_t reg, uint16_t val)
{
...
uint16_t AudioControlAC101::ReadReg(uint8_t reg)
{
	Wire.beginTransmission(AC101_ADDR);
	Wire.write(reg);
...
AudioControlAC101::AudioControlAC101()
{
}

bool AudioControlAC101::begin(int sda, int scl, uint32_t frequency)
{
	...
	ok &= WriteReg(ADC_SRC, 0x0); //0x2020);
	ok &= WriteReg(ADC_DIG_CTRL, 0x8000);
	ok &= WriteReg(ADC_APC_CTRL, 0xbbc6);
...
void AudioControlAC101::DumpRegisters()
{
...
uint8_t AudioControlAC101::GetVolumeSpeaker()
{
	// Times 2, to scale to same range as headphone volume
	return (ReadReg(SPKOUT_CTRL) & 31) * 2;
}

bool AudioControlAC101::SetVolumeSpeaker(uint8_t volume)
{
...
uint8_t AudioControlAC101::GetVolumeHeadphone()
{
	return (ReadReg(HPOUT_CTRL) >> 4) & 63;
}

bool AudioControlAC101::SetVolumeHeadphone(uint8_t volume)
{
...
bool AudioControlAC101::SetI2sSampleRate(I2sSampleRate_t rate)
{
	return WriteReg(I2S_SR_CTRL, rate);
}

bool AudioControlAC101::SetI2sMode(I2sMode_t mode)
{
...
bool AudioControlAC101::SetI2sWordSize(I2sWordSize_t size)
{
	uint16_t val = ReadReg(I2S1LCK_CTRL);
	val &= ~0x0030;
	val |= uint16_t(size) << 4;
...
bool AudioControlAC101::SetI2sFormat(I2sFormat_t format)
{
	uint16_t val = ReadReg(I2S1LCK_CTRL);
	val &= ~0x000C;
...
bool AudioControlAC101::SetI2sClock(I2sBitClockDiv_t bitClockDiv, bool bitClockInv, I2sLrClockDiv_t lrClockDiv, bool lrClockInv)
{
	uint16_t val = ReadReg(I2S1LCK_CTRL);
	val &= ~0x7FC0;
...
bool AudioControlAC101::SetMode(Mode_t mode)
{
	bool ok = true;
	if (MODE_LINE == mode)
	{
...
void AudioControlAC101::init(void)
{
    if(!configured)
    {
/*
        ESP_LOGI(TAG, "Configuring AC101...");
        if(begin(CODEC_SCK,CODEC_SDA));
        if (!ok) 
        {
            ESP_LOGE(TAG, "AC101 configuration error.");
        }
        else
        { 
            ESP_LOGI(TAG, "AC101 configured.");
        }
